...
The true PAG can be obtained by running the FCI with an oracle (i.e., conditional
independencies are directly inferred from the true DAG using d-separation).  
...
### Simulating data from the true SCM
...
### Identification via ID Algorithm
...
## Conditional Effect Identification from the Causal Diagram
...
### FCI algorithm for learning the PAG from observational data

Since our variables follow a Gaussian distribution, the partial correlation test (Fisher's Z test)
...
### Effect Identification via Backdoor / GAC 
...
### Effect Identification via IDP Algorithm 
...
### Conditional Effect Identification via CIDP Algorithm 
...
2. How to estimate the adjustment expression?
